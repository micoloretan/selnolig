...
\LuaLaTeX\ natively supports the so-called \textsc{utf-8} input encoding scheme. The \pkg{selnolig} package assumes that users make full use of this feature. Most modern \TeX-aware editors support \textsc{utf-8} directly; legacy files, however, may use other input encoding systems. If your input files currently use a different input encoding scheme, such as \textsc{latin1}, you should consider changing them over to \textsc{utf-8}. There are several methods for changing a file's input encoding scheme; see the question \href{http://tex.stackexchange.com/q/87406/5001}{How to change a .tex file's input encoding system (preferably to \textsc{utf-8})?} and the associated answers on \url{tex.stackexchage.com} for some possibilities.
...
\item The convention mentioned in the preceding bullet point could also be taken to imply that the~ft and~\char"E187\ ligatures may be used \emph{if} they occur at the very ends of words and word fragments\textemdash e.g., geschärft, Unbedarftheit, and gera\char"E187 (note that the word stems are schärfen, Bedarf, and raffen, which do not include the final~t)\textemdash but that they should be suppressed when -f-te, -f-ten, -f-tes, etc.\ suffixes are involved, as in gestreif\breaklig te, schlürf\breaklig tes\breaklig t, and raff\breaklig ten. All of these suffixes form syllables that start with~a~\enquote{t}. Note, though, the deliberate use of the expression \enquote{taken to imply} in the first sentence of this paragraph: I have so far \emph{not} encountered any kind of authoritative discussion of this particular typographic concern. For now, the \pkg{selnolig} package \emph{does not} break up the~ft and~\char"E187\ ligatures in these cases. However, I am willing to change this setting if confronted with appropriate typographic arguments.
