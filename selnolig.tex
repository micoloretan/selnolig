...
\item The |\nolig| search-and-insert operations currently don't work properly if the first part of the search string, i.e., the part \emph{before} the \Verb+|+~symbol, contains a character with an Umlaut (dieresis) or other diacritic. [Note added for version 0.159 of the package: This bug may be circumvented by moving the vertical bar (\Verb+|+) in the second argument of the affected \cmmd{nolig} statement forward (earlier) by one position. This fix is admittedly extremely kludgy, but it works!]
