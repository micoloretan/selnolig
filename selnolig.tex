...
According to their analysis, an initial version (ca.\ late October 2012) of the \pkg{selnolig} package dealt correctly with 85 percent (394,000) of those word forms, while still generating 21,000 Type~I errors and 48,000 Type~II errors; 1000 words contained more than one potential ligature, hence the difference.\footnote{In the context of the \pkg{selnolig} package, a Type~I error is the failure to suppress a typographically inappropriate ligature, and a Type~II error is the suppression of a ligature that is actually appropriate.}\textsuperscript{,}\footnote{However, they also discovered that a considerable part of these errors weren't real errors but, rather, results of typos in the corpus or of highly implausible morphological analyses.}
The detailed error lists they generated were critical in helping me refine the package's search patterns and bring the number of errors down drastically.\footnote{For instance, Steffen's creation of the \cmmd{keeplig} macro provided the means to eliminate, in one fell swoop, more than 10,000 [!] Type~II error cases that were generated by the package's incorrect suppression of the fl-ligature in words containing the strings \opt{Pflicht} and \opt{pflicht}.}
...
Felix and Steffen started the automated testing as their final project for the course Introduction to Computational Linguistics at the University of Massachusetts at Amherst in the fall of 2012. Their documentation and code are available online at [TBD]. They wish to thank the Institut für Maschinelle Sprachverarbeitung at the Universität Stuttgart for granting them a license for the morphological analysis tool SMOR and, in particular, Helmut Schmid for his guidance. They also express their gratitude to the \emph{Web-as-Corpus kool ynitiative} (\emph{WaCky}) for letting them use the SDeWaC corpus,%
...
\item If a word could \emph{end} with an fl-ligature (say, because of an abbreviation that's in effect) even though the~f and~l characters technically belong to different morphemes, the fl-ligature \emph{is} used. E.g., one writes Aufl.\ \emph{with} an fl ligature. [Note: I still need to fix a bug that prevents the use of the fl-ligature for Aufl.] But, when spelled out, the word should be typeset without the fl-ligature, i.e., as Auflage.
