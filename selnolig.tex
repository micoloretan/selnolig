...
\usepackage[ngerman,english,hdlig]{selnolig}
\debugon
...
\item if a prefix (acting as a derivational morpheme) and main word are joined: \mbox{offload} $\to$ offload, mi\mbox{st}rust~$\to$ mistrust, di\mbox{sp}lay~$\to$ display, etc. 
...
\item The ft ligature is also suppressed in words that end in |fth|, such as fifth and twelfth. For such words, the particle |th|\textemdash which in English is pronounced in a distinctive way\textemdash would be unhelpfully obscured if the ft ligature were not suppressed (\emph{viz.}, \mbox{fifth} and \mbox{twelfth}).
\end{itemize}

Unless the \opt{broad-f} option is set, the \pkg{selnolig} package will not suppress ligatures that arise if the main word ends in~f and the suffix\textemdash respresenting either an inflectional or a derivational morpheme\textemdash starts with an~i: elfin, selfish, golfing, surfing, beefier, fluffily, goofiness, standoffish, jiffies, buffiest. This is done because the slight gain in readability that may result from breaking up the fi and ffi ligatures would seem to be more than outweighed by the visual clash created between the~f (ff) and~i glyphs. 
