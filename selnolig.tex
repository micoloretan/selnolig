...
The remainder of this document is organized as follows. \Cref{sec:hurry} provides brief instructions for loading the \pkg{selnolig} package and, if necessary, making one's document(s) suitable for compilation under \LuaLaTeX. The package's overall approach to selective ligature suppression is explained in \cref{sec:approach}. The package's user macros are presented in \cref{sec:structure}, and options users may select to affect the package's workings are explained in \cref{sec:options}. \Cref{sec:further} discusses further issues that may arise when looking to break up typographic ligatures. The package's ligature suppression rules for English and German documents are listed in \cref{sec:eng-listing,sec:germ-listing}. The code of the package's main ``style'' file, \pkg{selnolig.sty}, is listed in \cref{sec:sty}, and the package's Lua code, contained in the file \pkg{selnolig.lua}, is shown in \cref{sec:luacode}. \Cref{sec:template} provides a suggested template for reporting bugs and other issues with the package.
...
\begin{Verbatim}
              ...
              \keeplig{flicht}
\end{Verbatim}
Observe that for the word |Lauflicht| the rule \Verb+\keeplig{flicht}+ undoes the action of the rule \Verb+\nolig{flich}{f|lich}+. However, because the string |aufl| is \emph{not} a subset of the string |flicht|, the rule |\keeplig{flicht}| does not undo the action of the rule \Verb+\nolig{aufl}{auf|l}+. Hence, the word |Lauflicht| happily ends up being typeset as Lauflicht, i.e., \emph{without} the fl-ligature. 

Interestingly, the rule |\keeplig{flicht}| is itself a bit too broad because it improperly catches the composite noun |Sumpflicht|, for which the fl-ligature \emph{should} in fact be suppressed. (Note that the word is composed of the free morphemes are |Sumpf| and |licht|, rather than |Sum| and |pflicht|!) To address this issue, the file \pkg{selnolig-german-patterns.sty} provides the rule \Verb+\nolig{Sumpfl}{Sumpf|l}+; this \cmmd{nolig} rule overrides the action of the rule |\keeplig{flicht}| for the word Sumpflicht. It also serves to suppress the fl ligature in words such as Sumpflabkraut and Sumpfleiche.
