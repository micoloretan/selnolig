...
To terminate the writing of the debugging-related information to the \opt{.log} file, one may execute the command
\begin{Verbatim}
     \debugoff
\end{Verbatim}
...
By default, selnolig's macros are switched on. If you want to suspend their operation for some part of the document, you can issue the command
\begin{Verbatim}
     \selnoligoff
\end{Verbatim}

And, if \Verb+\selnoligoff+ needs to be reversed at some later stage, i.e., if \pkg{selnolig}'s macros need to be switched back on, one may do so by issuing the command
\begin{Verbatim}
      \selnoligon
\end{Verbatim}
