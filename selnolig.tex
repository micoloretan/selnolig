...
The \pkg{selnolig} package suppresses typographic ligatures based on pre\-defined search patterns for English and German language documents. The search patterns focus on ligatures deemed inappropriate because they span morpheme boundaries. For example, the word \opt{shelfful}, which is mentioned in the \TeX book as a word for which the \enquote{ff} ligature might be inappropriate, is automatically typeset as shelfful rather than as shel\mbox{ff}ul.
\newline\hspace*{\parindent}
The \pkg{selnolig} package provides ligature suppression macros  for the \enquote{common} typographic f-ligatures. These comprise not only the familiar~ff, fi, fl, ffi, and~ffl ligatures but also ligatures such as \char"E18D\ and~\char"E187, which are frequently provided by fonts well suited for typesetting text.
\newline\hspace*{\parindent}
...
\newline\hspace*{\parindent}
The \cmmd{nolig} and \cmmd{keeplig} macros of the \pkg{selnolig} package \emph{require} the \LuaLaTeX\ format of a reasonably modern \TeX\ distribution such as \TeX Live\,2012, Mac\TeX\,2012, or MiK\TeX\,2.9.
\end{abstract}
...
The \cmmd{nolig} and \cmmd{keeplig} macros of the \pkg{selnolig} package \emph{require} use of the \LuaLaTeX\ format. They will \emph{not} work with pdf\LaTeX\ or \XeLaTeX. To run the package's ligature suppression macros, you \emph{must} compile your document under \LuaLaTeX.\footnote{One \emph{can} run the \pkg{selnolig} under either pdf\LaTeX\ or \XeLaTeX, but in that case only the package's hyphenation exception patterns will be loaded. The package will issue a warning message if it's not run under \LuaLaTeX but will not terminate completely.}

The \LuaLaTeX requirement will likely force you to make some changes to the preambles of your existing \LaTeX\ files in order to have them meet \LuaLaTeX's requirements. The changes should be fairly minor and straightforward to implement, because \LuaLaTeX\ is, for the most part, a strict superset of pdf\LaTeX. Almost all documents that compile correctly under pdf\LaTeX\ should also compile correctly under \LuaLaTeX\ as long as some required changes are made.
The main required changes are: (i) remove all \Verb+\usepackage{inputenc}+ and \Verb+\usepackage{fontenc}+ instructions from the preamble, and (ii)~insert the instruction
\Verb+\usepackage{fontspec}+.\footnote{If you do not load the \pkg{fontspec} package, you will be limited to using the Computer Modern fonts.}
...
|lualatex myfile| rather than |latex myfile| (or |pdflatex myfile|). If you use an editor with pull-down menus or buttons to invoke a suitable \TeX\ compiler for your document, be sure to select |LuaLaTeX|. 
